...
static void
present_wnmd_unflip(WindowPtr window)
{
    present_window_priv_ptr window_priv = present_window_priv(window);
    ...
    (*screen_priv->wnmd_info->unflip) (window, window_priv->unflip_event_id);
}
...
present_wnmd_check_flip_window (WindowPtr window)
{
    ...
    if (flip_pending) {
        ...
        if (!present_wnmd_check_flip(flip_active->crtc, flip_active->window, flip_active->pixmap, flip_active->sync_flip, NULL, 0, 0))
            present_wnmd_unflip(window);
    }

    /* Now check any queued vblanks */
    xorg_list_for_each_entry(vblank, &window_priv->vblank, window_list) {
        if (vblank->queued && vblank->flip && !present_wnmd_check_flip(vblank->crtc, window, vblank->pixmap, vblank->sync_flip, NULL, 0, 0)) {
            vblank->flip = FALSE;
            if (vblank->sync_flip)
                vblank->requeue = TRUE;
        }
